...
export default class Autopurge extends Command {
	...
	public async enable({ message, args }: core.CommandData) {
		...
		if (guild.id !== this.config.dynoGuild) {
			if ((interval < 240 || interval > 4320) && !this.config.isPremium) {
				return this.error(message.channel,
					`Purge interval must be more than 4 hours and no more than 3 days. Shorter intervals are available in RNet Premium.`);
		...
		if (!this.config.isPremium && docs && docs.length) {
			return this.error(message.channel,
				`You already have auto purge enabled on ${docs.length} channel(s). More channels are available in RNet Premium.`);
		}
