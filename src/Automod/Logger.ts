...
export default class Logger extends Base {
	...
	public log(e: LogEvent) {
		...
		if (!action) {
			...
			this.rnet.internalEvents.emit('automod', { type: e.type, guild: message.channel.guild });
		}
	...
	public createModlog(e: LogEvent, modlog: any) {
		const { message, guildConfig, msgContent, reason, action, filter } = e;

		let title = filter.delete ?
			`**Message sent by ${message.author.mention} deleted in ${message.channel.mention}**` :
			`**Message sent by ${message.author.mention} in ${message.channel.mention}**`;
		...
		return this.sendWebhook(modlog, { embeds: [embed] }, guildConfig)
			.then(() => {
				if (guildConfig && guildConfig.missingWebhooks) {
					...
					this.rnet.guilds.update(message.guild.id, { $unset: { missingWebhooks: 1 } }).catch(() => false);
				}
			})
			.catch(() => {
				if (guildConfig && !guildConfig.missingWebhooks) {
					guildConfig.missingWebhooks = 1;
					this.rnet.guilds.update(message.guild.id, { $set: { missingWebhooks: 1 } }).catch(() => false);
				}
