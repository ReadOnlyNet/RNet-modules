...
export default class Automod extends Module {
	...
	public start() {
		...
		this.moderation = new ModUtils(this.rnet);
	...
	public diagnose({ guild, guildConfig, diagnosis }: any) {
		...
		if (guildConfig.automod.channel) {
			...
			if (channel) {
				...
				diagnosis.issues.push(`I can't find the log channel, make sure the channel exists, and the RNet role has read/send messages.`);
			}
		} else {
