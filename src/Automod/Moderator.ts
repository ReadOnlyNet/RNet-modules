...
export default class Moderator extends Base {
	...
	public ban(guild: eris.Guild, message: eris.Message, reason: string = 'RNet Automod') {
		return this.client.banGuildMember(guild.id, message.member.id, 1, reason);
	}

	public async mute(message: eris.Message, guildConfig: rnet.GuildConfig, limitOverride?: number, reason?: string) {
		...
		reason = reason ? `RNet Automod - ${reason}` : `RNet Automod`;
		...
		try {
			...
			const clientUser = this.client.users.get(this.rnet.userid);
