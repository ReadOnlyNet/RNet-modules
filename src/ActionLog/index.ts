...
export default class ActionLog extends Module {
	...
	public start() {
		...
		this.rnet.commands.on('command', this.commandListener);
	...
	public unload() {
		this.rnet.commands.removeListener('command', this.commandListener);
	}
	...
	public messageCreate({ message, guildConfig }: any) {
		...
		.then(async (logChannel: eris.TextChannel) => {
			...
			if (message.guild.id === this.config.dynoGuild) {
				...
				if (inviteGuild) {
					embed.fields.push({ name: 'RNet', value: inviteGuild.deleted === true ? 'Kicked' : 'In Server', inline: true });
					...
					embed.fields.push({ name: 'RNet', value: 'Never Added', inline: true });
				}
			}
	...
	public guildMemberAdd({ guild, member, guildConfig }: any) {
		this.shouldLog({ event: 'guildMemberAdd', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildMemberRemove({ guild, member, guildConfig }: any) {
		this.shouldLog({ event: 'guildMemberRemove', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildBanAdd({ guild, member, guildConfig }: any) {
		this.shouldLog({ event: 'guildBanAdd', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildBanRemove({ guild, member, guildConfig }: any) {
		this.shouldLog({ event: 'guildBanRemove', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public messageUpdate({ message, oldMessage, guildConfig }: any) {
		...
		this.shouldLog({ event: 'messageEdit', guild: message.guild, channel: message.channel, guildConfig })
			.then((logChannel: eris.TextChannel) => {
				if (!logChannel) { return; }
	...
	public messageDelete({ message, guildConfig }: any) {
		...
		this.shouldLog({ event: 'messageDelete', guild, channel: channel, guildConfig }).then(async (logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public messageDeleteBulk({ channel, guild, messages, guildConfig }: any) {
		...
		this.shouldLog({ event: 'messageDeleteBulk', guild, channel, guildConfig }).then(async (logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public channelCreate({ channel, guildConfig }: any) {
		...
		this.shouldLog({ event: 'channelCreate', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public channelDelete({ channel, guildConfig }: any) {
		...
		this.shouldLog({ event: 'channelDelete', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildRoleCreate({ guild, role, guildConfig }: any) {
		this.shouldLog({ event: 'guildRoleCreate', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildRoleUpdate({ guild, role, oldRole, guildConfig }: any) {
		...
		this.shouldLog({ event: 'guildRoleUpdate', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildRoleDelete({ guild, role, guildConfig }: any) {
		this.shouldLog({ event: 'guildRoleDelete', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public guildMemberUpdate({ guild, member, oldMember, guildConfig }: any) {
		...
		if (member.roles !== oldMember.roles) {
			...
			if (addedRoles.length) {
				each(addedRoles, (roleId: string) => this.memberRoleAdd({ guild, member, roleId, guildConfig }));
			}
			if (removedRoles.length) {
				each(removedRoles, (roleId: string) => this.memberRoleRemove({ guild, member, roleId, guildConfig }));
			}
		}
	}
	...
	public nickChange({ guild, member, oldMember, guildConfig }: any) {
		this.shouldLog({ event: 'nickChange', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public userChange({ guild, user, oldUser, guildConfig }: any) {
		this.shouldLog({ event: 'userChange', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	 ...
	 * @returns {*}
	 */
	public memberRoleAdd({ guild, member, roleId, guildConfig }: any) {
		this.shouldLog({ event: 'memberRoleAdd', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	 ...
	 * @returns {*}
	 */
	public memberRoleRemove({ guild, member, roleId, guildConfig }: any) {
		this.shouldLog({ event: 'memberRoleRemove', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public voiceChannelJoin({ guild, member, channel, guildConfig }: any) {
		this.shouldLog({ event: 'voiceChannelJoin', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public voiceChannelLeave({ guild, member, channel, guildConfig }: any) {
		this.shouldLog({ event: 'voiceChannelLeave', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	public voiceChannelSwitch({ guild, member, channel, oldChannel, guildConfig }: any) {
		this.shouldLog({ event: 'voiceChannelSwitch', guild, guildConfig }).then((logChannel: eris.TextChannel) => {
			if (!logChannel) { return; }
	...
	private shouldLog(e: any): Promise<eris.GuildChannel> {
		...
		if (!guildConfig) {
			return new Promise((resolve: Function) => {
				this.rnet.guilds.getOrFetch(guild.id).then((config: rnet.GuildConfig) => {
					if (!config) {
						return resolve(null);
					}
	 ...
	 */
	private logEvent(channel: eris.TextChannel, embed: eris.EmbedBase, guildConfig: rnet.GuildConfig): Promise<any> {
		...
		if (!this.hasPermissions(channel.guild, 'manageWebhooks')) {
			...
			if (!skip) {
				...
				return this.sendMessage(channel, warning.join('\n'));
			}
