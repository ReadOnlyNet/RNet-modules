...
export default class ActionLog extends Command {
	...
	public ignorechannel({ message, args, guildConfig }: core.CommandData) {
		...
		return this.rnet.guilds.update(guild.id, { $set: { 'actonlog.ignoredChannels': guildConfig.actonlog.ignoredChannels } })
	...
	public ignored({ message, args, guildConfig }: core.CommandData) {
		...
		if (!guildConfig.actionlog.ignoredChannels) {
			return this.sendMessage(message.channel, `There are no channels or roles ignored by action log.`);
		}
