import {Base} from '@rnet.cf/rnet-core';
...
export default class Moderations extends Base {
	...
	public async process() {
		...
		this.utils.asyncForEach(docs, async (doc: any) => {
			...
			const guildConfig = await this.rnet.guilds.getOrFetch(doc.server);
			if (!guildConfig) {
				return;
			}
	...
	public ban(guild: eris.Guild, doc: ModerationDoc, guildConfig: rnet.GuildConfig) {
	...
	public async mute(guild: eris.Guild, doc: ModerationDoc, guildConfig: rnet.GuildConfig) {
		// check permissions.
		let member: eris.Member;

		if (guild && !guild.members) {
			try {
				member = await this.rnet.restClient.getRESTGuildMember(guild.id, doc.user.id);
			} catch (err) {
				return;
			}
	...
	// tslint:disable-next-line:cyclomatic-complexity
	public async role(guild: eris.Guild, doc: ModerationDoc, guildConfig: rnet.GuildConfig) {
		// check permissions.
		let member: eris.Member;
	...
	public async lock(guild: eris.Guild, doc: ModerationDoc, guildConfig: rnet.GuildConfig) {
