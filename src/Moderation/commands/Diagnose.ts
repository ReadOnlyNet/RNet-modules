...
export default class Diagnose extends Command {
	...
	public async execute(e: any) {
		...
		if (message.guild.id === this.config.dynoGuild && args.length > 1 && args[args.length - 1].match(/([0-9]+)/)) {
			if (this.isServerMod(message.member, message.guild.defaultChannel)) {
				const guildConfig = await this.rnet.guilds.getOrFetch(args.pop());
				if (!guildConfig) {
		...
		const command = this.rnet.commands.find((c: core.Command) => c.name === args[0] && c.permissions !== 'admin');

		if (command) {
			return this.diagnoseCommand(command, e);
		}

		const module = this.rnet.modules.find((m: core.Module) => !m.core &&
	...
	public diagnose({ message, guildConfig, perms }: any) {
		...
		if (!clientMember.roles || !clientMember.roles.length) {
			diagnosis.issues.push(`RNet does not have a role, it should have atleast the RNet role.`);
			diagnosis.issues.push(`You can fix this by authorizing the bot here: https://www.rnet.cf/invite`);
		}

		for (const perm of permissions) {
			if (!clientMember.permission.has(perm)) {
				const permission = this.config.permissionsMap[perm];
				diagnosis.issues.push(`The RNet role is missing the ${permission} permission.`);
			}
		}
		...
		if (perms && clientMember.roles && clientMember.roles.length) {
			...
			if (highestRole && highestRole.position === 1) {
				diagnosis.issues.push(`The RNet role has not been moved, move it up in the list above other user roles.`);
			}
		}
	...
	public diagnoseCommand(command: core.Command, { message, guildConfig }: any) {
		...
		if (globalConfig && globalConfig.commands && globalConfig.commands.hasOwnProperty(name) &&
			...
			diagnosis.issues.push(`The command is globally disabled in RNet by the developer.`);
		}

		if (globalConfig && globalConfig.modules && globalConfig.modules.hasOwnProperty(module) &&
			globalConfig.modules[module] === false) {
			diagnosis.issues.push(`The ${module} module is globally disabled in RNet by the developer.`);
		}
		...
		} else if (this.rnet.modules.has(module)) {
	...
	public diagnoseModule(module: core.Module, { message, guildConfig, remote }: any) {
		...
		if (globalConfig && globalConfig.modules && globalConfig.modules.hasOwnProperty(name) &&
			...
			diagnosis.issues.push(`The module is globally disabled in RNet by the developer.`);
		}
		...
		const embed = {
			...
			footer: {
				...
				text: `${this.config.stateName} | Cluster ${this.rnet.options.clusterId} | Shard ${message.channel.guild.shard.id} | ID ${message.channel.guild.id}`,
			},
		};
