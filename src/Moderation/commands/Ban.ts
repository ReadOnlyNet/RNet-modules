import {Command, SubCommand} from '@rnet.cf/rnet-core';
...
export default class Ban extends Command {
	...
	public async match({ message, args, guildConfig }: core.CommandData) {
		...
		const Moderation = this.rnet.modules.get('Moderation');
	...
	private async ban({ message, args, guildConfig, preserve }: any) {
		...
		try {
			...
			if (!user && args[0].match(/^([0-9]+)$/)) {
				...
				try {
					const res = await modUtils.banMember(guild, message, args[0], guildConfig, reason, limit);
