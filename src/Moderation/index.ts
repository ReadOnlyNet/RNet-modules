...
export default class Moderation extends Module {
	 ...
	 */
	public start() {
		this.modUtils = new ModUtils(this.rnet);
		...
		this.models.Moderation.find({ server: guild.id, userid: member.id }).lean().exec()
			.then((docs: any[]) => {
				for (const doc of docs) {
					...
					member.addRole(role.id, encodeURIComponent('RNet role persist')).catch(() => false);
				}
			});
	}

	public async logEvent(event: any) {
		const guildConfig = await this.rnet.guilds.getOrFetch(event.guild.id);
		if (!guildConfig) {
			return;
		}
