import {Base, Role} from '@rnet.cf/rnet-core';
import * as eris from '@rnet.cf/eris';
import * as each from 'async-each';
import * as rnet from 'RNet';
import * as moment from 'moment';

const helpText = 'Make sure the RNet role has `Manage Roles` and `Manage Channels` permissions.';
const permError = `I don't have enough permissions. ${helpText}`;
const modError = `That user is a mod/admin, I can't do that.`;
const hierarchyError = `My role isn't high enough to moderate this user. Move the RNet role up above other roles.`;
...
export default class ModUtils extends Base {
	 ...
	 * Create muted role
	 */
	public async createMutedRole(guild: eris.Guild, guildConfig: rnet.GuildConfig, _role?: Role): Promise<eris.Role> {
		const options = {
			name: 'Muted',
			color: this.utils.hexToInt('#818386'),
		};

		_role = _role || new Role(this.rnet, guild);
		...
		this.rnet.guilds.update(guild.id, { $set: { 'moderation.mutedRole': role.id } })
	...
	public async log(event: ModerationEvent) {
		...
		this.rnet.guilds.update(guild.id, { $inc: { 'moderation.count': 1 } })
			.catch((err: any) => this.logger.error(err, {
		...
		if (guildConfig.moderation.channel) {
			...
			const embed = {
				...
				fields: [
					...
					{ name: 'Moderator', value: mod ? mod.mention : `<@${this.rnet.userid}>`, inline: true },
				],
	...
	public async channelLog(event: ModerationEvent) {
		...
		const embed = {
			...
			fields: [
				...
				{ name: 'Moderator', value: user ? user.mention : `<@${this.rnet.userid}>`, inline: true },
			],
	...
	public async muteMember(
		...
		const _role = new Role(this.rnet, guild);
		_role.createOverwritePermissions(role.id, [...guild.channels.values()], [
			'sendMessages',
			'addReactions',
			'voiceSpeak',
	 ...
	 */
	public deleteCommand(message: eris.Message, guildConfig: rnet.GuildConfig) {
	 ...
	 */
	public isProtected(message: eris.Message, member: eris.Member, guildConfig: rnet.GuildConfig) {
	...
	public getResponse(response: string, guildConfig: rnet.GuildConfig, reason: string): string {
	...
	public getCustomResponse(user: string, key: string, guildConfig: rnet.GuildConfig, reason: string): string {
