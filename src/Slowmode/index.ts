...
export default class Slowmode extends Module {
	...
	public enable(guildConfig: rnet.GuildConfig, channel: eris.GuildChannel, time: number, user: boolean) {
		...
		return this.rnet.guilds.update(guild.id, { $set: { slowmode: guildConfig.slowmode } });
	}

	public disable(guildConfig: rnet.GuildConfig, channel: eris.GuildChannel) {
		const guild = (<eris.GuildChannel>channel).guild;
		guildConfig.slowmode = guildConfig.slowmode || {};
		guildConfig.slowmode.channels = guildConfig.slowmode.channels || [];
