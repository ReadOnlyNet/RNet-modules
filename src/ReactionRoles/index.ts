...
export default class ReactionRoles extends Module {
	...
	public messageReactionAdd({ message, emoji, userId, guild, guildConfig }: any) {
		...
		this.client.addGuildMemberRole(guild.id, userId, reactionRole.roleId, 'RNet Reaction Roles').catch(() => null);

		this.rnet.internalEvents.emit('reactionroles', { type: 'add', guild: guild });
	}
	...
	public messageReactionRemove({ message, emoji, userId, guild, guildConfig }: any) {
		...
		const reactionRole = msgConfig.roles.find(r => (emoji.id && r.emojiId === emoji.id) || r.native === emoji.name);
		if (!reactionRole) { return; }
		this.client.removeGuildMemberRole(guild.id, userId, reactionRole.roleId, 'RNet Reaction Roles').catch(() => null);

		this.rnet.internalEvents.emit('reactionroles', { type: 'remove', guild: guild });
	}
}
